...
LDFLAGS := -static
LOADER_FEATURE_FLAGS :=
ifeq ($(SELF_DELETE),1)
LOADER_FEATURE_FLAGS += -DHARMLESS_SELF_DELETE
endif
ifeq ($(MASQUERADE_ARGV0),1)
LOADER_FEATURE_FLAGS += -DHARMLESS_MASQUERADE_ARGV0
endif
...
$(ARM64_LOADER_BIN): $(LOADER_SOURCES) | $(ARM64_BUILD_DIR)
	$(TARGET_CC) $(TARGET_CFLAGS) $(STEALTH_FLAGS) $(LOADER_FEATURE_FLAGS) $(TARGET_ARCH_FLAGS) $(TARGET_OPENSSL_CFLAGS) $(INCLUDES) -o $@ $^ $(TARGET_OPENSSL_LDFLAGS) 2>/dev/null || $(TARGET_CC) $(TARGET_CFLAGS) $(STEALTH_FLAGS) $(LOADER_FEATURE_FLAGS) $(TARGET_ARCH_FLAGS) $(TARGET_OPENSSL_CFLAGS) $(INCLUDES) -o $@ $^ $(TARGET_OPENSSL_LDFLAGS) -lzstd -lz
...
$(ARM32_LOADER_BIN): $(LOADER_SOURCES) | $(ARM32_BUILD_DIR)
	$(ARM32_CC) $(ARM32_TARGET_CFLAGS) $(STEALTH_FLAGS) $(LOADER_FEATURE_FLAGS) $(ARM32_TARGET_ARCH_FLAGS) $(ARM32_OPENSSL_CFLAGS) $(INCLUDES) -o $@ $^ $(ARM32_OPENSSL_LDFLAGS) 2>/dev/null || $(ARM32_CC) $(ARM32_TARGET_CFLAGS) $(STEALTH_FLAGS) $(LOADER_FEATURE_FLAGS) $(ARM32_TARGET_ARCH_FLAGS) $(ARM32_OPENSSL_CFLAGS) $(INCLUDES) -o $@ $^ $(ARM32_OPENSSL_LDFLAGS) -lzstd -lz
